...
function sellTickets(queue) {
  ...
  const notEnoughChange = queue.some((bill) => {
...
function sortCitiesArray(arr) {
  return arr.sort((a, b) => {
    const countryComparision = a.country.localeCompare(b.country);
    if (countryComparision !== 0) {
      return countryComparision;
    }
...
class CSSSelector {
  ...
  pseudoElement(value) {
    this.checkUnique('pseudoElementName');
    this.checkOrder(6);
  ...
  static combine(selector1, combinator, selector2) {
    ...
    combined.combinedSelectors = [selector1, combinator, selector2];
    return combined;
  }
  ...
  stringify() {
    if (this.combinedSelectors.length) {
      return this.combinedSelectors
        .map((selector) =>
          typeof selector === 'string' ? selector : selector.stringify()
        )
        .join(' ');
    }
    return (
